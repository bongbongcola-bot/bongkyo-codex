...
function buildPhotoMarkers(photoList) {
  ...
  return photoList.map((photo, index) => `[사진 ${index + 1}: ${photo.name}]`);
}
...
function buildBodyParagraphs(place, keyword, author, features, photoMarkers) {
  const photoLead = photoMarkers[0]
    ? `${photoMarkers[0]} 제가 처음 카메라를 들고 담았던 장면인데요, ${place}의 첫인상과 전체 분위기를 보여주기에 딱 좋은 컷이라고 느꼈어요. 현장에서 처음 마주한 느낌을 자연스럽게 전해주기 좋은 사진이더라고요.`
  ...
  const middlePhoto = photoMarkers[1]
    ? `${photoMarkers[1]} 두 번째 사진은 제가 현장에서 특히 눈여겨봤던 공간이나 포인트를 보여주는 장면이에요. 글을 읽다가 이 사진을 같이 보면 제가 왜 이 분위기를 좋게 느꼈는지 더 잘 전달되더라고요.`
    : `${keyword} 검색해서 들어오시는 분들은 사진이랑 같이 실제 분위기를 보고 싶어 하시니까, 제가 현장에서 느꼈던 인상을 본문 중간에 같이 적어주는 방식이 잘 맞아요.`;

  const extraPhotos = photoMarkers.slice(2).length
    ? `${photoMarkers.slice(2).join(" ")} 추가 사진들은 제가 현장에서 지나치기 아까웠던 디테일이나 분위기를 담은 컷들이에요. 이런 장면들은 글 설명 뒤에 붙여주면 실제 촬영 후기를 읽는 느낌이 더 살아나더라고요.`
...
function buildHtmlOutput(data) {
  ...
  const photoMarkers = buildPhotoMarkers(photos);
  const paragraphs = buildBodyParagraphs(place, keyword, author, features, photoMarkers);
  const photoList = photos.length
    ? photos.map((photo, index) => `    <li>사진 ${index + 1}: ${escapeHtml(photo.name)}</li>`).join("\n")
    : "    <li>등록된 샘플사진 없음</li>";
...
function buildMarkdownOutput(data) {
  ...
  const paragraphs = buildBodyParagraphs(place, keyword, author, features, photoMarkers);
  const photoList = photos.length
    ? photos.map((photo, index) => `- 사진 ${index + 1}: ${photo.name}`).join("\n")
    : "- 등록된 샘플사진 없음";
...
function buildTextOutput(data) {
  ...
  const paragraphs = buildBodyParagraphs(place, keyword, author, features, photoMarkers);
  const photoList = photos.length
    ? photos.map((photo, index) => `${index + 1}. 사진 ${index + 1}: ${photo.name}`).join("\n")
    : "1. 등록된 샘플사진 없음";
  ...
  previewGallery.innerHTML = uploadedPhotos
    .map((photo, index) => `
      <div class="preview-item">
        ...
        <span>사진 ${index + 1}: ${escapeHtml(photo.name)}</span>
      </div>
    `)
    .join("");
...
function updatePreview(files) {
  ...
  Promise.all(
    ...
    uploadedPhotos = photos.filter((photo) => photo.dataUrl);
    renderPreviewGallery();
  });
}
